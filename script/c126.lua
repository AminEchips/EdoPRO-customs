...
function s.initial_effect(c)
	...
	c:EnableReviveLimit()
	Fusion.AddProcMix(c,true,true,s.fusionfilter,s.pendfilter,s.pendfilter)
	...
	-- Board wipe if material was in Pendulum Zone
	...
	-- Register End Phase effect if destroyed
...
function s.pendfilter(c,fc,sumtype,tp)
	return c:IsAttribute(ATTRIBUTE_DARK,fc,sumtype,tp) and c:IsType(TYPE_PENDULUM,fc,sumtype,tp)
end

-- Effect 1: Add "Fusion" or "Polymerization" Spell on Fusion Summon
...
-- Effect 2: Board wipe if Pendulum Zone materials used
function s.wipecon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local mat=c:GetMaterial()
	return mat:IsExists(function(tc) return tc:IsPreviousLocation(LOCATION_PZONE) end,1,nil)
end
...
-- Effect 3: Register revival effect if destroyed by card effect
...
-- Revival: Special Summon 1 Dragon from GY or face-up Extra Deck
