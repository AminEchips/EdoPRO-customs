...
function s.initial_effect(c)
	...
	--Negate opponent's activation during your turn
	...
	--Revive during End Phase if something you controlled was sent to GY this turn
...
function s.negcost(e,tp,eg,ep,ev,re,r,rp,chk)
	return true -- optional effect, no cost
end
...
function s.chainlimit(e,rp,tp)
	return tp==rp -- prevent opponent from chaining to this
end
function s.negop(e,tp,eg,ep,ev,re,r,rp)
	Duel.NegateActivation(ev)
end

--Track if card you controlled was sent to GY by opponent
	...
	ge1:SetOperation(function(_,tp,eg,ep,ev,re,r,rp)
		if rp~=tp then
			for tc in eg:Iter() do
				if tc:IsPreviousControler(tp) and tc:IsPreviousPosition(POS_FACEUP) and tc:IsPreviousLocation(LOCATION_ONFIELD) then
