...
-- Filter: "Elemental HERO"-mentioning Spell in GY
function s.setfilter(c)
	return c:IsType(TYPE_SPELL) and c:IsSSetable() and c:CheckText("Elemental HERO")
end

-- Filter: banished "Elemental HERO" monsters
function s.hero_filter(c)
	return c:IsSetCard(0x8) and c:IsType(TYPE_MONSTER) and c:IsFaceup() and c:IsAbleToHand()
end
...
function s.op(e,tp,eg,ep,ev,re,r,rp)
	...
	local op=e:GetLabel()
	if op==0 then
		-- Set 1 Spell that mentions "Elemental HERO" from GY
		...
		-- Special Summon this card, then add 1 banished "Elemental HERO" if valid
