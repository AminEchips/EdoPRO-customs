...
function s.initial_effect(c)
	...
	-- Destroy 1 Spell/Trap (targeting)
...
-- Fusion filter: 2 Elemental HERO Fusion Monsters
function s.fusionfilter(c,fc,sumtype,tp)
	return c:IsFusionSetCard(0x3008) and c:IsType(TYPE_FUSION,fc,sumtype,tp)
end

-- Effect 1: Targeted Spell/Trap destruction
function s.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and chkc:IsSpellTrap() end
	if chk==0 then return Duel.IsExistingTarget(aux.FilterBoolFunction(Card.IsSpellTrap),tp,0,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,aux.FilterBoolFunction(Card.IsSpellTrap),tp,0,LOCATION_ONFIELD,1,1,nil)
