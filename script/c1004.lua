...
function s.initial_effect(c)
	...
	-- Return a monster to hand; then Special Summon this + Token
...
-- Valid target to bounce
function s.tgfilter(c)
	return c:IsFaceup() and (c:IsSetCard(0x16d) or c:IsRace(RACE_WYRM)
		or c:IsCode(68468459) or (c.ListsCode and c:ListsCode(68468459)))
		and c:IsAbleToHand()
end
	...
	if Duel.SendtoHand(tc,nil,REASON_EFFECT)>0 and c:IsRelateToEffect(e) then
		...
		if Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)~=0 then
			...
			if Duel.IsPlayerCanSpecialSummonMonster(tp,id+1,0,TYPES_TOKEN,0,0,4,RACE_WYRM,ATTRIBUTE_WATER,tp) then
				...
				-- Restrict to Synchros from Extra Deck while token is up
				local e1=Effect.CreateEffect(c)
