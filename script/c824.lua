...
function s.initial_effect(c)
    ...
    -- Effect 2: Detached from DARK Xyz Monster to activate effect
    ...
    -- Effect 3: GY effect – banish this card, boost "The Phantom Knights" ATK
...
-- Effect 1: Discard 1 card as cost
...
-- Effect 2: Condition – detached from DARK Xyz to activate its effect
function s.reccon(e,tp,eg,ep,ev,re,r,rp)
    local c=e:GetHandler()
    return c:IsReason(REASON_COST) and r&REASON_COST~=0
        and c:IsReason(REASON_EFFECT) and r&REASON_EFFECT~=0
        and c:IsPreviousLocation(LOCATION_OVERLAY)
        and c:GetReasonCard() and c:GetReasonCard():IsAttribute(ATTRIBUTE_DARK)
end

-- Target banished "Phantom Knights" Trap
...
-- Effect 3: GY effect – boost all "The Phantom Knights" you control
