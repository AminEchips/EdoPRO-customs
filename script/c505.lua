...
function s.initial_effect(c)
    ...
    --Synchro Summon (Opponent's turn only)
...
--Effect 1: Synchro Summon
function s.synfilter(c)
    return c:IsType(TYPE_SYNCHRO) and c:IsRace(RACE_SPELLCASTER) and Duel.IsExistingMatchingCard(Card.IsCanBeSynchroMaterial,tp,LOCATION_MZONE,0,1,nil,c)
...
function s.matfilter(c)
    return c:IsAbleToRemove() and (c:IsLevelAbove(1) or c:IsType(TYPE_LINK))
end
function s.ritg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(s.ritfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,1,nil)
        and Duel.IsExistingMatchingCard(s.matfilter,tp,LOCATION_HAND+LOCATION_MZONE,0,1,nil) end
end
function s.riop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local mat=nil
    local function levelplus(c) return c:IsType(TYPE_LINK) and c:GetLink() or c:GetLevel() end
    local chkf=tp
    local result=false
    aux.GCheckAdditional=aux.RitualCheckAdditional(rc,lv,"Equal")
    local mat=Duel.SelectRitualMaterial(tp,rc,mg,nil,lv,"Equal",chkf)
