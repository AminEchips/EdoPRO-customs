...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then
		...
		local canToken=hasZone and Duel.IsPlayerCanSpecialSummonMonster(tp,TOKEN_SWORDSOUL,0x16d,TYPES_TOKEN|TYPE_TUNER,0,0,4,RACE_WYRM,ATTRIBUTE_WATER) and Duel.IsExistingMatchingCard(s.spswordsoulfilter,tp,LOCATION_MZONE,0,1,nil)
		return canSummonSelf or canToken
	end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local hasZone=Duel.GetLocationCount(tp,LOCATION_MZONE)>0
	local canSummonSelf=hasZone and c:IsRelateToEffect(e) and Duel.IsExistingMatchingCard(s.sptunerfilter,tp,LOCATION_MZONE,0,1,nil)
	local canToken=hasZone and Duel.IsPlayerCanSpecialSummonMonster(tp,TOKEN_SWORDSOUL,0x16d,TYPES_TOKEN|TYPE_TUNER,0,0,4,RACE_WYRM,ATTRIBUTE_WATER) and Duel.IsExistingMatchingCard(s.spswordsoulfilter,tp,LOCATION_MZONE,0,1,nil)
	if not (canSummonSelf or canToken) then return end

	local opt=0
	if canSummonSelf and canToken then
		opt=Duel.SelectOption(tp,aux.Stringid(id,0),aux.Stringid(id,1))
	elseif canSummonSelf then
		opt=0
	elseif canToken then
		opt=1
	end
