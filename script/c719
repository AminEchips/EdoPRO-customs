...
function s.initial_effect(c)
    ...
    -- Effect 2: On Special Summon if you control LIGHT or DARK Noble Knight Synchro/Xyz
...
s.listed_names={03580032,10736540,82140600,24027078} -- Merlin, Lady of the Lake, Avalon cards
...
-- Effect 2: Check condition on Special Summon
function s.synxyzfilter(c)
    return c:IsFaceup() and c:IsSetCard(0x107a) and
        (c:IsType(TYPE_SYNCHRO) or c:IsType(TYPE_XYZ)) and
        (c:IsAttribute(ATTRIBUTE_LIGHT) or c:IsAttribute(ATTRIBUTE_DARK))
end
...
function s.atkop(e,tp,eg,ep,ev,re,r,rp)
    ...
    -- Second attack next Battle Phase
    ...
    -- Return to Extra Deck when it leaves field
