...
function s.initial_effect(c)
    -- On Special Summon: add 2 "Noble Knight" cards from GY to hand
    ...
    -- Tribute + Summon "Emperor Charles"
...
-- Effect 1: Add 2 "Noble Knight" cards from GY to hand
...
-- Effect 2: Tribute + Summon "Emperor Charles"
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(Card.IsDiscardable,tp,LOCATION_HAND,0,1,nil) end
    Duel.DiscardHand(tp,Card.IsDiscardable,1,1,REASON_COST+REASON_DISCARD)
end
function s.trbfilter(c)
    return c:IsFaceup() and c:IsType(TYPE_MONSTER) and c:GetEquipGroup():GetCount()>0 and c:IsReleasable()
end
function s.spfilter(c,e,tp)
    return c:IsCode(77656797) and c:IsCanBeSpecialSummoned(e,0,tp,true,false)
end
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.GetLocationCountFromEx(tp,tp,nil,TYPE_MONSTER)>0
