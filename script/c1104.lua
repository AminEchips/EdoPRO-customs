...
function s.initial_effect(c)
	...
	c:RegisterEffect(e1)
	-- Reincarnation Summoned: send and copy name
...
-- Effect 1: On Ritual Summon
function s.thcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsSummonType(SUMMON_TYPE_RITUAL)
end
function s.thfilter(c)
	return (c:IsCode(38784726) or (c:IsSetCard(0x119) and c:IsType(TYPE_RITUAL))) and c:IsAbleToHand()
end
...
-- Effect 2: Name copy if Reincarnation Summoned
...
function s.namecost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.costfilter,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_EXTRA,0,1,nil) end
