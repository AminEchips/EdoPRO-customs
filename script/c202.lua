...
-- Effect 1: Fusion Summon an "Elemental HERO" Fusion Monster
function s.filter(c,e,tp,m,f,chkf)
    return c:IsSetCard(0x8) and c:IsType(TYPE_FUSION) and c:IsLevelAbove(1)
...
function s.fustg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then
        ...
        mg1:Merge(mg2)
        return Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,chkf)
    end
...
function s.fusop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local mg2=Duel.GetMatchingGroup(Card.IsAbleToGrave,tp,LOCATION_DECK,0,nil)
    mg1:Merge(mg2)
    local sg=Duel.GetMatchingGroup(s.filter,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,chkf)
    if #sg>0 then
        Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
        local tc=sg:Select(tp,1,1,nil):GetFirst()
        if tc then
            local mat=Fusion.SelectFusionMaterial(tp,tc,mg1,nil,chkf)
            tc:SetMaterial(mat)
...
-- Effect 2: Banish, recycle 3 "HERO" Fusion Monsters, draw 1
