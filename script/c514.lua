...
function s.negcon(e,tp,eg,ep,ev,re,r,rp)
    return re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) and Duel.IsChainNegatable(ev)
        and re:IsActiveType(TYPE_SPELL+TYPE_TRAP+TYPE_MONSTER) and re:IsHasCategory(CATEGORY_DESTROY)
end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
    ...
    local tc=g:GetFirst()
    if tc then
        if tc:IsCanBeSpecialSummoned(e,0,tp,false,false) and (not tc:IsAbleToHand() or Duel.SelectYesNo(tp,aux.Stringid(id,2))) then
