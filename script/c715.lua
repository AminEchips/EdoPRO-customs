...
function s.initial_effect(c)
	...
	-- Special Summon 4 monsters from 4 different locations
...
function s.ctfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0x107a)
		and c:IsSummonType(SUMMON_TYPE_SPECIAL) and c:GetSummonLocation()==LOCATION_EXTRA
...
function s.cttg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.SetOperationInfo(0,CATEGORY_COUNTER,nil,1,0,COUNTER_HOLY)
end
function s.ctop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) then
		c:AddCounter(COUNTER_HOLY,1)
	end
end

-- Can only activate summon effect with 5 counters
...
-- Filters for 4 specific archetypes and locations
