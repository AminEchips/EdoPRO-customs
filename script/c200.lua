--Elemental HERO Cosmic Soldier (Debug Version)
local s,id=GetID()
...
-- First effect: send Neos or Neo-Spacian from Deck to GY (excluding self)
...
function s.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
    ...
    Debug.Message("Paying Special Summon cost...")
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
    ...
    Debug.Message("Special Summon executing...")
    if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)>0 and tc then
        ...
        e2:SetValue(attr)
        c:RegisterEffect(e2)
        Debug.Message("Copied code: "..code.." and attribute: "..attr)
    end
end

-- Second effect: tribute another HERO or Neo-Spacian (not self)
function s.cfilter(c,e)
    return c:IsFaceup() and (c:IsSetCard(0x8) or c:IsSetCard(0x1f)) and c~=e:GetHandler()
end
function s.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
    Debug.Message("Checking tribute cost...")
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
    Debug.Message("Adding Polymerization to hand...")
