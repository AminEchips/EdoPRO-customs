...
function s.initial_effect(c)
    ...
    e1:SetCode(EVENT_FREE_CHAIN)  -- Activation from hand
    c:RegisterEffect(e1)

    -- Effect 1: Send Fusion Monster to GY, Special Summon 1 "Neo-Spacian" with the same Attribute
    ...
    -- Effect 2: Shuffle Fusion Monsters that mention "Elemental HERO Neos" into Extra Deck
    ...
    e4:SetRange(LOCATION_GRAVE)  -- Operates from GY
    e4:SetCountLimit(1)
...
function s.target1(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then
        -- Check for Fusion Monsters and Neo-Spacians with the same Attribute in the GY
...
function s.target2(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then
        -- Look for Fusion Monsters mentioning "Elemental HERO Neos"
...
function s.target3(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then
        ...
        return Duel.IsExistingMatchingCard(Card.IsCode,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil,42015635)
    end
