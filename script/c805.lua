...
function s.initial_effect(c)
	...
	--Move to Pendulum Zone if destroyed or sent as cost for DARK monster's effect
...
function s.pzcon(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsReason(REASON_DESTROY) then return true end
	if not (c:IsReason(REASON_COST) and re and re:IsActivated() and re:IsMonsterEffect()) then return false end
	local rc=re:GetHandler()
	if rc:IsRelateToEffect(re) and rc:IsFaceup() then
		return rc:IsAttribute(ATTRIBUTE_DARK)
	else
		return Duel.GetChainInfo(ev,CHAININFO_TRIGGERING_ATTRIBUTE)&ATTRIBUTE_DARK~=0
	end
end
