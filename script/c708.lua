...
function s.initial_effect(c)
	...
	--Effect 1: On Synchro Summon, equip up to 2 Equip Spells from GY if used equipped monster
	...
	--Effect 2: Send 1 LIGHT Noble Knight, destroy 1 Spell/Trap, gain 1000 ATK
...
--Effect 1: If Synchro Summoned using monster with Equip
function s.eqcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsSummonType(SUMMON_TYPE_SYNCHRO) then return false end
	local mat=c:GetMaterial()
	return mat:IsExists(function(tc) return tc:GetEquipCount()>0 end,1,nil)
end
...
--Effect 3: End Phase negate if sent to GY this turn
