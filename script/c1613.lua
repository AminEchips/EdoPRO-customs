...
function s.initial_effect(c)
	...
	-- 2. Redirect attack and negate Sleipnir permanently (Quick Effect)
...
-- Odin monster codes (explicit only)
local odin_ids = {
	[93483212]=true, [1621]=true, [1647]=true
}

-- 1. Check if Synchro Summoned with Nordic Beast Tuner
function s.indcon(e)
...
-- 2. Check if opponent declares attack and you control an Aesir monster
...
function s.atkop(e,tp,eg,ep,ev,re,r,rp)
	...
	-- Change attack target to Sleipnir
	Duel.ChangeAttackTarget(c)
	-- Immediately and permanently negate Sleipnir
	if c:IsFaceup() then
		local e1=Effect.CreateEffect(c)
		e1:SetType(EFFECT_TYPE_SINGLE)
...
-- 3. Equip to Odin monster after destruction
...
function s.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	c:RegisterEffect(e1)
	-- If the equipped monster attacks a monster: lock both from leaving field by card effect
	local e2=Effect.CreateEffect(c)
