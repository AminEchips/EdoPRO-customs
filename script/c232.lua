...
function s.initial_effect(c)
	c:EnableReviveLimit()
	--Summon Token on Normal or Special Summon
	...
	-- Place Supreme King's Castle if banished by card that mentions Dark Fusion
	local e3=Effect.CreateEffect(c)
	e3:SetDescription(aux.Stringid(id,1))
	e3:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e3:SetCode(EVENT_REMOVE)
	e3:SetProperty(EFFECT_FLAG_DELAY)
	e3:SetCondition(s.placon)
	e3:SetTarget(s.plactg)
	e3:SetOperation(s.placop)
	c:RegisterEffect(e3)
end
...
function s.tkop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SpecialSummon(token,0,tp,tp,false,false,POS_FACEUP)>0 then
		-- Restrict non-HERO Special Summons
...
function s.plactg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0 
		and Duel.IsExistingMatchingCard(s.pfilter,tp,LOCATION_DECK,0,1,nil) end
end
function s.pfilter(c)
	return c:IsCode(72043279) and not c:IsForbidden() and c:IsType(TYPE_FIELD)
end
function s.placop(e,tp,eg,ep,ev,re,r,rp)
