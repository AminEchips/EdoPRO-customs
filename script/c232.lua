...
function s.initial_effect(c)
    -- Token summon on Normal/Special Summon
    ...
    -- Place "Supreme King's Castle" if banished by effect of card that mentions "Dark Fusion"
...
function s.tkop(e,tp,eg,ep,ev,re,r,rp)
    ...
    -- Lock Special Summons except "HERO"
...
-- Check if banished by a card that mentions "Dark Fusion"
function s.placecon(e,tp,eg,ep,ev,re,r,rp)
    local rc=re:GetHandler()
    return rc and rc:IsCode(94820406)
end
function s.placetg(e,tp,eg,ep,ev,re,r,rp,chk)
    if chk==0 then return Duel.IsExistingMatchingCard(s.fieldfilter,tp,LOCATION_DECK,0,1,nil) end
end
function s.placeop(e,tp,eg,ep,ev,re,r,rp)
    local tc=Duel.GetFirstMatchingCard(s.fieldfilter,tp,LOCATION_DECK,0,nil)
    if tc then
